...
pub fn update_tray_menu(app: &AppHandle, state: &TrayIconState, locale: Option<&str>) {
    ...
    let selected_is_cloud = crate::cloud_providers::is_cloud_model(current_model_id);

    let mut downloaded: Vec<_> = models.into_iter().filter(|m| m.is_downloaded).collect();
    downloaded.sort_by(|a, b| {
        let a_cloud = crate::cloud_providers::is_cloud_model(&a.id);
        let b_cloud = crate::cloud_providers::is_cloud_model(&b.id);
        a_cloud.cmp(&b_cloud).then_with(|| a.name.cmp(&b.name))
    });
    ...
    let unload_model_i = MenuItem::with_id(
        ...
        model_loaded && !selected_is_cloud,
        None::<&str>,
    )
