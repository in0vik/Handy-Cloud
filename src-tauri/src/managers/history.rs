...
use crate::audio_toolkit::{read_wav_samples, save_wav_file};
...
impl HistoryManager {
    ...
    fn map_history_entry(row: &rusqlite::Row<'_>) -> rusqlite::Result<HistoryEntry> {
        Ok(HistoryEntry {
            ...
            cloud_pending: row.get("cloud_pending").unwrap_or(false),
        })
    }
    ...
    /// Save a history entry for an already-written WAV file whose cloud
    /// transcription should be retried later.
    pub fn save_pending_entry(
        ...
        debug!("Saved pending history entry with id {}", entry.id);
    ...
    pub fn update_transcription(
        ...
        let entry = conn
            .query_row(
                "SELECT id, file_name, timestamp, saved, title, transcription_text, post_processed_text, post_process_prompt, post_process_requested, cloud_pending
    ...
    pub fn update_pending_transcription(
        &self,
        id: i64,
        transcription_text: String,
    ) -> Result<HistoryEntry> {
    ...
    pub fn get_audio_samples(&self, file_name: &str) -> Result<Vec<f32>> {
        read_wav_samples(&self.get_audio_file_path(file_name)).map_err(Into::into)
    }
